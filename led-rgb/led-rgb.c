...
bool ledButtonStatus = NOT_PRESSED;
...
unsigned char currentColor = TURN_OFF;
...
int main()
{
    ...
    while (true)
    {
        ...
        else if (ledButtonStatus == PRESSED)
        {
            switch (currentColor)
            {
            case TURN_OFF:
...
void initializeComponents()
{
    ...
    gpio_set_dir(LED_BUTTON_PIN, IN);
    gpio_pull_up(LED_BUTTON_PIN);
...
void readButtons()
{
    ledButtonStatus = gpio_get(LED_BUTTON_PIN);
